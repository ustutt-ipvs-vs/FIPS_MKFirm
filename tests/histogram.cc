...
TEST_F(HistogramTest, UniformHistogram) {
  ...
  PDB wired_pdb(network, network[{1, 2}], {100, 200});
  EXPECT_EQ(wired_pdb.ifg.delay, DelayInterval(0, 960));
  EXPECT_EQ(wired_pdb.propagation, 50);
  EXPECT_EQ(wired_pdb.serialization, DelayInterval(8000, 16000));
  EXPECT_EQ(wired_pdb.processing, DelayInterval(0));
  EXPECT_EQ(wired_pdb.wireless, DelayInterval(0));
  EXPECT_EQ(wired_pdb.d_trans, DelayInterval(8000, 16960));
  EXPECT_EQ(wired_pdb.d_total, DelayInterval(8050, 17010));

  PDB wireless_pdb(network, network[{1, 2}], {100, 200}, delay_histogram, 0.5,
                   MINIMIZE_DMAX);
  ...
  EXPECT_EQ(wireless_pdb.d_trans,
            DelayInterval(8000, 16960 + N / 2 * TicksPerMilliSec));
